// Source: https://raw.githubusercontent.com/yumeng5/JoSH/master/src/josh.c
...
char train_file[MAX_STRING], output_file[MAX_STRING], res_file[MAX_STRING];
char tree_emb_file[MAX_STRING], matrix_file[MAX_STRING], level_file[MAX_STRING];
...
long long train_words = 0, word_count_actual = 0, iter = 5, pretrain_iter = 0, file_size = 0, iter_count;
real alpha = 0.025, starting_alpha, sample = 1e-3, global_lambda = 1.5, lambda_tree = 1.0, lambda_cat = 1.0;
real word_margin = 0.25, tree_margin = 0.2, tree_margin_sub = 0.2, cat_margin = 0.2;
...
// number of nodes and levels in the tree
...
void UpdateRoot() {
  int i = 0, j, a;
  for (a = 0; a < layer1_size; a++) tree[i].emb[a] = 0;
  for (j = 0; j < tree[i].num_children; j++) {
    int child = tree[i].children[j];
    for (a = 0; a < layer1_size; a++) tree[i].emb[a] += tree[child].emb[a];
  }
  real norm = 0.0;
...
void ReadCategoryName() {
  ...
  while ((read = getline(&line, &len, f)) != -1) {
    ...
    for (j = 0; j <= read; j++) {
      if (line[j] == ' ' || j == read) {
        strncpy(tmp_word, line + st, j - st);
        tmp_word[j - st] = 0;
        if (flag == 0) {
          if (atoi(tmp_word) != i) {
            printf("[ERROR] Category ID %d wrong!\n", atoi(tmp_word));
            exit(1);
          }
          flag++;
        } else if ((vocab_idx = SearchVocab(tmp_word)) != -1 && i > 0) {
          tree[i].cur_words[tree[i].init_size++] = vocab_idx;
          if (flag == 1) {
            strcpy(tree[i].node_name, tmp_word);
            flag++;
          }
        } else if (i > 0) {
    ...
    tree[i].cur_size = tree[i].init_size;
  ...
  if (i != nodes) {
    printf("[ERROR] Wrong number of categories!\n");
    exit(1);
  }

  for (i = 1; i < nodes; i++) {
    if (tree[i].init_size == 0) {
  ...
  for (i = 1; i < nodes; i++) {
    printf("(#%lld): ", i);
    for (j = 0; j < tree[i].init_size; j++) {
      printf("%s ", vocab[tree[i].cur_words[j]].word);
    }
    printf("\n");
  }
}
...
void ReadTaxo() {
  ...
  FILE *f_mat = fopen(matrix_file, "rb");
  printf("Taxonomy matrix file: %s\n", matrix_file);
  if (f_mat == NULL) {
    printf("Taxonomy matrix file not found!\n");
    exit(1);
  }
  ...
  if (f_level == NULL) {
    printf("Node level file not found!\n");
    exit(1);
  }
  fscanf(f_mat, "%lld", &nodes);
  ...
  for (i = 0; i < nodes; i++) {
    ...
    tree[i].init_size = 0;
    tree[i].grad = (real *)calloc(layer1_size, sizeof(real));
    tree[i].wt_score = (real *)calloc(vocab_size, sizeof(real));
    tree[i].relation_id = (int *)calloc(nodes, sizeof(int));
    tree[i].siblings = (int *)calloc(nodes, sizeof(int));
    tree[i].num_siblings = 0;
    tree[i].children = (int *)calloc(nodes, sizeof(int));
    tree[i].num_children = 0;
    tree[i].node_id = i;
    for (j = 0; j < nodes; j++) {
      fscanf(f_mat, "%d", &tree[i].relation_id[j]);
      if (tree[i].relation_id[j] == 1) {
        tree[i].siblings[tree[i].num_siblings++] = j;
      }
      else if (tree[i].relation_id[j] == 2) {
        tree[j].children[tree[j].num_children++] = i;
        tree[i].parent = j;
      }
    }
    fscanf(f_level, "%d", &tree[i].level);
    if (tree[i].level > max_level) max_level = tree[i].level;
  }
  level_margin = (real *)calloc(max_level, sizeof(real));
  UpdateRoot();
...
void UpdateMargin() {
  ...
  // zero out margins
  for (c = 0; c < max_level; c++) level_margin[c] = 0;
  int *level_counts = (int *)calloc(max_level, sizeof(int));

  for (u = 0; u < nodes; u++) {
    if (tree[u].num_children > 0) {
      for (v = 0; v < tree[u].num_children; v++) {
        child0 = tree[u].children[v];
        h = 0;
        for (c = 0; c < layer1_size; c++) h += tree[u].emb[c] * tree[child0].emb[c];
        for (vp = 0; vp < tree[u].num_children; vp++) {
          if (vp != v) {
            child1 = tree[u].children[vp];
            g = 0;
            for (c = 0; c < layer1_size; c++) g += tree[child0].emb[c] * tree[child1].emb[c];
            g = h - g;
            level_counts[tree[u].level] += 1;
            level_margin[tree[u].level] += g;
          }
        }
      }
    }
  }
  for (c = 0; c < max_level; c++) {
    level_margin[c] /= level_counts[c];
    printf("Set level %d tree margin as %2f\n", c, level_margin[c]);
  }
  free(level_counts);
}
...
real TreeEmb() {
  ...
  for (u = 1; u < nodes; u++) {
    f = 0;
    cur_margin = level_margin[tree[u].level - 1];
    for (c = 0; c < layer1_size; c++) f += tree[u].emb[c] * tree[tree[u].parent].emb[c];
    for (a = 0; a < tree[u].num_siblings; a++) {
      cnt++;
      v = tree[u].siblings[a];
      h = 0;
      ...
      if (f - h < cur_margin) {
        ...
        for (c = 0; c < layer1_size; c++) grad[c] += tree[tree[u].parent].emb[c] - f * tree[u].emb[c] 
                                                  + h * tree[u].emb[c] - tree[v].emb[c];
        for (c = 0; c < layer1_size; c++) tree[u].grad[c] += alpha * lambda_tree / tree[u].num_siblings * grad[c];
        
        // parent node gradient
        for (c = 0; c < layer1_size; c++) grad[c] = tree[u].emb[c] - f * tree[tree[u].parent].emb[c];
        for (c = 0; c < layer1_size; c++) tree[tree[u].parent].grad[c] += alpha * lambda_tree / tree[u].num_siblings * grad[c];

        // sibling node gradient
        for (c = 0; c < layer1_size; c++) grad[c] = h * tree[v].emb[c] - tree[u].emb[c];
        for (c = 0; c < layer1_size; c++) tree[v].grad[c] += alpha * lambda_tree / tree[u].num_siblings * grad[c];
      }
    }
  }
  ...
  for (i = 1; i < nodes; i++) {
    ...
    for (j = 0; j < tree[i].cur_size; j++) {
      ...
      if (f < cat_margin) {
        ...
        for (c = 0; c < layer1_size; c++) tree[i].grad[c] += alpha * lambda_cat / tree[i].cur_size * grad[c];
    ...
    for (c = 0; c < layer1_size; c++) tree[i].emb[c] += tree[i].grad[c];
    g = 0;

...
struct vocab_word {
  long long cn; // cn = count: The word frequency (number of times it appears).
...
void *TrainModelThread(void *id) {
  ...
  for (iter=0; iter < numiters; ++iter) {
     ...
     while (1) {
        // Update progress for each 10k pairs
        // also adjusts the training 'alpha' parameter.
        ...
        // Stop if there is no more pairs
        ...
        // Of input word or output word is OOV then skip them
        ...
        // negative là số lượng mẫu âm tính được train
        for (d = 0; d < negative + 1; d++) {
           ...
           // On the other iterations, we'll train random negative samples.
           } else {
              ...
              // Trường hợp trên không xảy ra do samplingtable được init từ vocab.output
              // vốn đã dc lọc đi các tokens không có nghĩa
           ...
           // Hết các bước trên lấy được `1` mẫu dương tính (label = 1) là output
           // và `negative` mẫu âm tính (label = 0) ngẫu nhiên từ bảng lấy mẫu
